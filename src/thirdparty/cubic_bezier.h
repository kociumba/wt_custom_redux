...
// credit: https://github.com/chromium/chromium/blob/main/ui/gfx/geometry/cubic_bezier.h
...
class CubicBezier {
   public:
    ...
    double SolveWithEpsilon(double x, double epsilon) const {
        if (x < 0.0) return ToFinite(0.0 + start_gradient_ * x);
        if (x > 1.0) return ToFinite(1.0 + end_gradient_ * (x - 1.0));
