// credit: https://github.com/ocornut/imgui/issues/1901#issuecomment-552185000
#ifndef PROGRESS_HPP
#define PROGRESS_HPP
...
static float bezier(float t) { return fast_out_slow_in.Solve(t); }

namespace ImGui {
constexpr static auto lerp(float x0, float x1) {
    return [=](float t) { return (1 - t) * x0 + t * x1; };
}

constexpr static float lerp(float x0, float x1, float t) { return lerp(x0, x1)(t); }

static auto interval(float T0, float T1, std::function<float(float)> tween = lerp(0.0, 1.0)) {
    return [=](float t) { return t < T0 ? 0.0f : t > T1 ? 1.0f : tween((t - T0) / (T1 - T0)); };
}
...
inline bool Spinner(const char* label, float radius, int thickness, const ImU32& color) {
    ...
    auto step_tween = [=](float t) { return floor(lerp(0.0, (float)num_detents, t)); };
    ...
    const float a_min = start_angle + tail_value * max_arc +
                        rotation_value * rotation_compensation - step_value * step_offset;
